...
This build covers **Phase 1 and Phase 2**: design system, public marketing site, application shell, database schema with Row-Level Security, authentication, the initial role model, the client onboarding wizard (language, profile, consent, journey selection, urgency/safety screening), and the full 28-section naturalization questionnaire engine with conditional logic and autosave. See `MVP_ROADMAP.md` for what's next.
...
3. Run `supabase/seed/seed.sql`, then `supabase/seed/naturalization_content_seed.sql` (in that order) to load reference data and the full naturalization questionnaire content (28 sections, ~76 questions, conditional rules, onboarding urgency-screening risk rules). Neither creates any user accounts.
...
src/
  ...
  pages/
    ...
    app/         Client dashboard, questionnaire engine, Reviewer/Admin dashboards, role placeholders
  types/         Shared TypeScript types (roles)
supabase/
  migrations/    Numbered SQL migrations, one concern per file, all RLS-enabled
  seed/          Reference-data seed + full naturalization questionnaire content seed (no user accounts)
```
